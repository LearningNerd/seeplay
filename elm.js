...
var author$project$Update$getCurrentCorrectNote = F2(
	function (index, arr) {
		var maybeCorrectNote = A2(elm$core$Array$get, index, arr);
		if (maybeCorrectNote.$ === 'Nothing') {
			return author$project$Note$createNote(60);
		} else {
			var correctNote = maybeCorrectNote.a;
			return correctNote;
		}
	});
...
var author$project$Update$restartTimer = F2(
	function (model, currentTimestamp) {
		...
		var correctNote = A2(author$project$Update$getCurrentCorrectNote, model.nextTargetNoteIndex, model.targetNotes);
		var newScoreObjectList = (!_Utils_eq(model.startTimestamp, elm$core$Maybe$Nothing)) ? _List_fromArray(
			[
				A3(author$project$Model$Score, correctNote, newAnswerSpeed, model.incorrectTries)
			]) : _List_Nil;
